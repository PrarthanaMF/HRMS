...
const QuickAccess = () => {
    ...
    return (
        // Added max-w-350 and mx-auto to center the whole section on big screens
        <div className='mt-1 w-full max-w-350 mx-auto'>
            {/* Added px-4 to create the left/right gap on mobile */}
            ...
            {/* 
              Added px-4 (padding left/right) to create the gap around the cards.
              Kept the responsive grid classes.
            */}
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 justify-center gap-3 md:gap-6 mt-2 px-4 md:px-6'>
                {data.map((row, index) => {
                    if (index === 0) return null;
                    return <ManageHomeAccount data={row} key={index} />
                })}
            </div>
        </div>
    )
...
function ManageHomeAccount({ data }) {
    return (
        <div className='bg-white border border-gray-200/80 rounded-xl px-2 py-3 md:px-4 flex flex-col items-center justify-between duration-300 transform hover:translate-y-1 cursor-pointer w-full'>
